...
public class NakedUmlEclipseContext{
		...
		new Job("Loading Opium Metadata"){
			...
			protected IStatus run(final IProgressMonitor monitor){
				...
				try{
					...
					return new Status(IStatus.OK, NakedUmlEclipsePlugin.getId(), "Opium Metadata loaded");
				}catch(Exception e){
					return new Status(IStatus.ERROR, NakedUmlEclipsePlugin.getId(), "Opium Metadata not loaded", e);
				}finally{
					isLoading = false;
					monitor.done();
				}
		...
		try{
			...
			if(dew == null){
				if(emfWorkspaces.size() > 0){
					...
					NakedUmlConfig cfg = getEmfToNakedUmlSynchronizer().getConfig();
	...
	public static void setCurrentContext(NakedUmlEclipseContext currentContext){
		NakedUmlEclipseContext.currentContext = currentContext;
	}
}
