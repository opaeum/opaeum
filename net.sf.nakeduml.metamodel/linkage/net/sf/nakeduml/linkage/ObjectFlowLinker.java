...
@StepDependency(phase = LinkagePhase.class, after = { NakedParsedOclStringResolver.class }, requires = { NakedParsedOclStringResolver.class })
public class ObjectFlowLinker extends AbstractModelElementLinker {
	@VisitBefore
	public void visitExpansionNode(INakedExpansionNode node) {
		INakedClassifier baseType = calculateBaseType(node.getIncoming());
		if (baseType != null) {
	...
	private INakedClassifier calculateBaseType(Collection<INakedActivityEdge> source) {
		...
		if (iter.hasNext()) {
			...
			if (flow.getSource() instanceof INakedObjectNode) {
				if (flow.getSource() instanceof INakedParameterNode || flow.getSource() instanceof INakedOutputPin) {
					...
					return calculateBaseType(flow.getSource().getIncoming());
				}
