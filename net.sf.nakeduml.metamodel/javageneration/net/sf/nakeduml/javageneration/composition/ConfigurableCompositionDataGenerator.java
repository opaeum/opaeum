...
public class ConfigurableCompositionDataGenerator extends AbstractTestDataGenerator{
	...
	private boolean couldBePersistedInModel(NakedStructuralFeatureMap map){
		INakedClassifier nakedBaseType = map.getProperty().getNakedBaseType();
		if(isPersistent(nakedBaseType)){
			return true;
		}else{
			return GeneralizationUtil.getConcreteEntityImplementationsOf(nakedBaseType, getModelInScope()).size() > 0;
		}
	}
	...
	private void populateToOneOrManyToMany(OJAnnotatedClass test,boolean hasOneToOneToOwner,INakedEntity entity,OJBlock populate){
		for(INakedProperty f:entity.getEffectiveAttributes()){
			NakedStructuralFeatureMap map = new NakedStructuralFeatureMap(f);
			if(f instanceof INakedProperty){
