...
public class MetaDefAnnotator extends AbstractJavaProducingVisitor {
	...
	public void visitInterface(INakedInterface i) {
		...
		OJAnnotatedPackage p = (OJAnnotatedPackage) super.findOrCreatePackage(OJUtil.packagePathname(i.getNameSpace()));
		createTextPathIfRequired(p, getOutputRoot());
		p.putAnnotation(metaDef);
		metaDef.putAttribute("name", HibernateUtil.metadefName(i));
	...
	public void visitCallAction(INakedCallOperationAction a) {
		if (a.getOperation().isUserResponsibility()) {
			...
			if (metaDef == null) {
				...
				OJAnnotatedPackage p = (OJAnnotatedPackage) super.findOrCreatePackage(OJUtil.classifierPathname(message));
				createTextPathIfRequired(p, getOutputRoot());
				p.putAnnotation(metaDef);
