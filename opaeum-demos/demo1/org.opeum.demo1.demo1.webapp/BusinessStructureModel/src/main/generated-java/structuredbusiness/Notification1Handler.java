...
public class Notification1Handler implements ISignalEventHandler {
	@Transient
	...
	private Notification1 signal = new Notification1();
	@Transient
	private HashSet<INotificationReceiver> to;

	/** Constructor for Notification1Handler
	 * 
	 * @param signal 
	 * @param isEvent 
	 */
	public Notification1Handler(Notification1 signal, boolean isEvent) {
		this.signal=signal;
		this.firstOccurrenceScheduledFor=new Date(System.currentTimeMillis()+1000);
		this.isEvent=isEvent;
	}
	
	/** Default constructor for Notification1Handler
	 */
	public Notification1Handler() {
	}
	...
	public String getQueueName() {
		return "structuredbusiness::Notification1";
	}
	...
	public boolean handleOn(Object targets) {
		...
		for ( Object target : (Collection<?>)targets ) {
			if ( target instanceof Notification1Receiver ) {
				if ( isEvent ) {
					consumed |=((Notification1Receiver)target).consumeNotification1Event(signal);
				} else {
					((Notification1Receiver)target).receiveNotification1(signal);
					consumed = true;
				}
			}
		}
