...
@AccessType(	"field")
@Table(name="organization_i_business_component_1",schema="opaeum_bpm")
...
public class Organization_iBusinessComponent_1 implements IPersistentObject, HibernateEntity, CompositionNode, Serializable {
	...
	private OrganizationalNode representedOrganization;
	 ...
	 * @param end2 
	 */
	public Organization_iBusinessComponent_1(IBusinessComponent end1, OrganizationalNode end2) {
	 ...
	 * @param end1 
	 */
	public Organization_iBusinessComponent_1(OrganizationalNode end2, IBusinessComponent end1) {
		this.z_internalAddToBusinessComponent(end1);
		this.z_internalAddToRepresentedOrganization(end2);
	}
	...
	public OrganizationalNode getRepresentedOrganization() {
		OrganizationalNode result = this.representedOrganization;
		...
		while ( i<propertyNodes.getLength() ) {
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("representedOrganization") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("5468800036735903732")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					Node currentPropertyValueNode = propertyValueNodes.item(j++);
					if ( currentPropertyValueNode instanceof Element ) {
						setRepresentedOrganization((OrganizationalNode)map.get(((Element)currentPropertyValueNode).getAttribute("uid")));
					}
				}
			}
		}
