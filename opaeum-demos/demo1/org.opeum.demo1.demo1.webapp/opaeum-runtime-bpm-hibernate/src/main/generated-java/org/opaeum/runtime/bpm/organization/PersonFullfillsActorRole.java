...
@AccessType(	"field")
@Table(name="person_fullfills_actor_role",schema="opaeum_bpm")
...
public class PersonFullfillsActorRole implements IPersistentObject, HibernateEntity, CompositionNode, Serializable {
	...
	private Person representedPerson;
	 ...
	 * @param end1 
	 */
	public PersonFullfillsActorRole(IBusinessActor end2, Person end1) {
	 ...
	 * @param end2 
	 */
	public PersonFullfillsActorRole(Person end1, IBusinessActor end2) {
		this.z_internalAddToRepresentedPerson(end1);
		this.z_internalAddToBusinessActor(end2);
	}
	...
	public Person getRepresentedPerson() {
		Person result = this.representedPerson;
	...
	public void init(CompositionNode owner) {
		this.z_internalAddToRepresentedPerson((Person)owner);
	}
		...
		while ( i<propertyNodes.getLength() ) {
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("representedPerson") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("6815035603503332095")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					Node currentPropertyValueNode = propertyValueNodes.item(j++);
					if ( currentPropertyValueNode instanceof Element ) {
						setRepresentedPerson((Person)map.get(((Element)currentPropertyValueNode).getAttribute("uid")));
					}
				}
			}
		}
