 ...
 * @generated
 */
public class NewBpmn2Diagrams extends Wizard implements INewWizard{
	/**
	 * @generated
	 */
	...
	public void init(IWorkbench workbench,IStructuredSelection sel){
		...
		setDefaultPageImageDescriptor(Bpmn2ImageRegistry.getImageDescriptor("NEW_PAGE_WZD"));
		setDialogSettings(Bpmn2Plugin.getDefault().getDialogSettings());
		setWindowTitle("Create new Bpmn2 diagrams");
	}
	/**
		...
		if(diagPage.isPageComplete()){
			if(diagPage.isNewModel()){
				...
				if(createdFile != null && result){
					...
					try{
						IDE.openEditor(Bpmn2Plugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage(), createdFile, true);
	...
	public void addPages(){
		diagPage = new Bpmn2DiagramsPage("New BPM2 Model Editor Diagram", selection);
		diagPage.setTitle("Bpmn2 Diagrams");
	...
	private boolean createModelFile(){
		try{
			...
			Bpmn2Plugin.log(ce);
			Bpmn2Plugin.displayDialog(null, "An error occured during the template generation.", IStatus.ERROR);
			return false;
		}
		return true;
	}
