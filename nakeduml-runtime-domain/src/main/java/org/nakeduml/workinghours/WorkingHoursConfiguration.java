...
public class WorkingHoursConfiguration extends WorkingHoursConfigurationGEN {
	...
	public static List allInstances() {
		return Collections.singletonList(getInstance());
	}
	...
	private boolean isOnceOffHoliday(Calendar date) {
		...
		while (iter.hasNext()) {
			...
			if (Calendar.MONDAY == date.get(Calendar.DAY_OF_WEEK)) {
				...
				if ((holiday.get(Calendar.YEAR) == date.get(Calendar.YEAR))
						&& (holiday.get(Calendar.DAY_OF_YEAR) + 1 == date.get(Calendar.DAY_OF_YEAR))) {
					return true;
				}
			}
		}
	...
	/**
	 * @return the amount of minutes since 00:00 on the morning of the day
	 *         represented by c
	 */
	...
	public int getBusinessHoursPerWeek() {
		if (super.getBusinessHoursPerWeek() == 0) {
			super.setBusinessHoursPerWeek(getBusinessHoursPerDay() * 5
					+ (getWorkDay(WorkDayType.SATURDAY).getMinutesPerDay())
