...
public class ForwardHandler7251280809563715157 implements ICallEventHandler {
	...
	public Collection<PropertyValue> marshall(Environment env) {
		...
		result.add(new PropertyValue(3350895467208403091l, Value.valueOf(this.getToPerson(),env)));
		return result;
	}
	...
	public void unmarshall(Collection<PropertyValue> ps, AbstractPersistence persistence) {
		for ( PropertyValue p : ps ) {
			if ( p.getId()==3350895467208403091l ) {
				this.setToPerson((IBusinessRole)Value.valueOf(p.getValue(),(InternalHibernatePersistence)persistence));
			} else {
