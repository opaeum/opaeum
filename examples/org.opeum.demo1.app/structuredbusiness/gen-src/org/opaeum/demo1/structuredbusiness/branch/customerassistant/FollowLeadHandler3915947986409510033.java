...
public class FollowLeadHandler3915947986409510033 implements ICallEventHandler {
	 ...
	 * @param isEvent 
	 */
	public FollowLeadHandler3915947986409510033(OnlineCustomer customer, Date timeOfLead, boolean isEvent) {
	...
	public void unmarshall(Collection<PropertyValue> ps, AbstractPersistence persistence) {
		for ( PropertyValue p : ps ) {
			if ( p.getId()==5501124320077697795l ) {
				this.setCustomer((OnlineCustomer)Value.valueOf(p.getValue(),(InternalHibernatePersistence)persistence));
			} else {
				if ( p.getId()==3684593961093410895l ) {
					this.setTimeOfLead((Date)Value.valueOf(p.getValue(),(InternalHibernatePersistence)persistence));
				} else {
