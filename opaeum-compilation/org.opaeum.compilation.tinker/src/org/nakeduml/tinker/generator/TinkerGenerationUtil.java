...
public class TinkerGenerationUtil{
	...
	public static final OJPathName tinkerSetClosableSequenceImplPathName = new OJPathName(
			"org.nakeduml.tinker.collection.TinkerSetClosableSequenceImpl");
	...
	public static String constructTinkerCollectionInit(OJAnnotatedClass owner,PropertyMap map,boolean jsf){
		if(map.getProperty().getOtherEnd() != null){
			return map.umlName() + " = new " + (map.getProperty().isOrdered() ? "TinkerArrayList" : jsf ? "TinkerJsfHashSet" : "TinkerHashSet")
					+ "<" + map.javaBaseTypePath().getLast() + ">(" + map.javaBaseTypePath().getLast() + ".class, this, " + owner.getName()
					+ ".class.getMethod(\"addTo" + NameConverter.capitalize(map.umlName()) + "\", new Class[]{" + map.javaBaseTypePath().getLast()
			...
			return map.umlName() + " = new " + (map.getProperty().isOrdered() ? "TinkerEmbeddedArrayList" : "TinkerEmbeddedHashSet") + "<"
					+ map.javaBaseTypePath().getLast() + ">(this, " + owner.getName() + ".class.getMethod(\"addTo"
					+ NameConverter.capitalize(map.umlName()) + "\", new Class[]{" + map.javaBaseTypePath().getLast() + ".class}), "
					+ owner.getName() + ".class.getMethod(\"removeFrom" + NameConverter.capitalize(map.umlName()) + "\", new Class[]{"
					+ map.javaBaseTypePath().getLast() + ".class}))";
		}
	}
	...
	public static OJBlock instantiateClassifier(OJBlock block,PropertyMap map){
		OJField field = new OJAnnotatedField(map.umlName(), map.javaBaseTypePath());
		...
		ojTryStatement.getTryPart().addToStatements(
				map.umlName() + " = (" + map.javaBaseTypePath().getLast() + ") c.getConstructor(Vertex.class).newInstance(v)");
	...
	public static String addSetterForSimpleType(PropertyMap map,boolean audit){
		if(map.getBaseType() instanceof Enumeration){
			return "this." + (audit ? "auditVertex" : "vertex") + ".setProperty(\""
					+ TinkerGenerationUtil.tinkeriseUmlName(map.getProperty().getQualifiedName()) + "\", val!=null?val.name():null)";
		}else{
			return "this." + (audit ? "auditVertex" : "vertex") + ".setProperty(\""
					+ TinkerGenerationUtil.tinkeriseUmlName(map.getProperty().getQualifiedName()) + "\", val==null?\"" + TINKER_DB_NULL + "\":val)";
		}
	}
