...
public class TinkerAuditOrignalClassTransformation extends AbstractAuditJavaProducingVisitor {
	...
	private void addCreateAuditVertexWithAuditEdge(INakedEntity c, OJAnnotatedClass ojClass) {
		...
		createAuditVertexWithAuditEdge.getBody().addToStatements("this.auditVertex = "+ TinkerGenerationUtil.graphDbAccess +".addVertex(\""+TinkerGenerationUtil.getClassMetaId(ojClass)+"Audit\")");
		...
		createAuditVertexWithAuditEdge.getBody().addToStatements("this.auditVertex.setProperty(\"transactionNo\", "+TinkerGenerationUtil.graphDbAccess+".getTransactionCount())");
		...
		createAuditVertexWithAuditEdge.getBody().addToStatements("auditEdgeToOriginal.setProperty(\"transactionNo\", "+TinkerGenerationUtil.graphDbAccess+".getTransactionCount())");
...
//				}
	...
	private void addCreateEdgeToOne(OJAnnotatedClass originalClass) {
		...
		ifOneInEdgeAllreadyExist.addToThenPart("Edge auditParentEdge = " + TinkerGenerationUtil.graphDbAccess + ".addEdge(null, oneAuditVertex, getAuditVertex(), label)");
		...
		ifOneOutEdgeAllreadyExist.addToThenPart("Edge auditParentEdge = " + TinkerGenerationUtil.graphDbAccess + ".addEdge(null, getAuditVertex(), oneAuditVertex, label)");
		ifOneOutEdgeAllreadyExist.addToThenPart("auditParentEdge.setProperty(\"outClass\", "+TinkerGenerationUtil.TINKER_GET_CLASSNAME+" + \"" + TinkerAuditGenerationUtil.AUDIT + "\")");
	...
	private void implementGetAudits(OJAnnotatedClass originalClass, INakedEntity c) {
		...
		OJPathName auditPath = new OJPathName(c.getMappingInfo().getQualifiedJavaName() + TinkerAuditGenerationUtil.AUDIT);
