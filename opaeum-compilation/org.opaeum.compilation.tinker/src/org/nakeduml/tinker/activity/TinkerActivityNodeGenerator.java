...
public class TinkerActivityNodeGenerator extends StereotypeAnnotator {
	...
	private void addGetBodyExpression(OJAnnotatedClass actionClass, INakedOclAction oa) {
		...
		if (oa.getBodyExpression()!=null) {
			getBodyExpression.getBody().addToStatements("return " + ValueSpecificationUtil.expressValue(getBodyExpression, new NakedValueSpecificationImpl(oa.getBodyExpression()), oa.getActivity(), oa.getReturnPin().getType()));
	...
	public void visitControlNode(INakedControlNode controlNode) {
		...
		case ACTIVITY_FINAL_NODE:
			controlNodeClass = createFinalNodeClass(controlNode, new OJPathName(TinkerBehaviorUtil.tinkerFinalNodePathName.toJavaString() + "<" + tokenType + ">"));
			break;
		case INITIAL_NODE:
			controlNodeClass = createInitialNodeClass(controlNode);
			break;
		case FORK_NODE:
			controlNodeClass = createDecisionOrForkNodeClass(controlNode, new OJPathName(TinkerBehaviorUtil.tinkerForkNodePathName.toJavaString() + "<" + tokenType + ">"));
			break;
		case MERGE_NODE:
			controlNodeClass = createMergeNodeClass(controlNode, new OJPathName(TinkerBehaviorUtil.tinkerMergeNodePathName.toJavaString() + "<" + tokenType + ">"));
			break;
		case JOIN_NODE:
			controlNodeClass = createJoinNodeClass(controlNode, new OJPathName(TinkerBehaviorUtil.tinkerJoinNodePathName.toJavaString() + "<" + tokenType + ">"));
			break;
		...
		case DECISION_NODE:
			...
			controlNodeClass = createDecisionOrForkNodeClass(controlNode, new OJPathName(TinkerBehaviorUtil.tinkerDecisionNodePathName.toJavaString() + "<" + tokenType + ">"));
			break;
		default:
			break;
		}
	...
	private void addInstantiateToken(OJAnnotatedClass nodeClass, INakedControlNode controlNode) {
		OJAnnotatedOperation instantiateToken = new OJAnnotatedOperation("instantiateToken");
		TinkerGenerationUtil.addOverrideAnnotation(instantiateToken);
		instantiateToken.setReturnType(determineTokenType(controlNode));
		instantiateToken.addToParameters(new OJParameter("name", new OJPathName("String")));
		instantiateToken.getBody().addToStatements("return new " + instantiateToken.getReturnType().getLast() + "(name)");
	...
	private void createControlFlowEdgeClass(INakedActivityEdge edge) {
		...
		if (edge instanceof INakedObjectFlow) {
			...
			superClass.addToGenerics(OJUtil.classifierPathname(((INakedObjectFlow) edge).getOriginatingObjectNode().getNakedBaseType()));
	...
	private void addSource(OJAnnotatedClass controlFlowEdge, INakedActivityEdge edge) {
		...
		OJPathName returnType = TinkerBehaviorUtil.tinkerActivityNodePathName.getCopy();
	...
	private void createVariablesForInputPins(OJClass actionClass, INakedAction oa) {
		for (INakedInputPin inputPin : oa.getInput()) {
			if (inputPin.getNakedMultiplicity().isOne()) {
				...
				pinVariable.setName(inputPin.getName());
				...
				setterForPinVariable.getBody().addToStatements("this.vertex.setProperty(\"" + inputPin.getName() + "\", val==null?\"__NULL__\":val)");
				...
				ifResultNull.addToThenPart("result = (" + OJUtil.classifierPathname(inputPin.getNakedBaseType()).getLast() + ") this.vertex.getProperty(\"" + inputPin.getName() + "\")");
				ifResultNull.addToThenPart("result = (result==null || result.equals(\"__NULL__\"))?null:result");
				getterForPinVariable.getBody().addToStatements(ifResultNull);
	...
	private void addInputPinGetters(OJClass actionClass, INakedAction a) {
		...
		for (INakedInputPin inputPin : inputPins) {
			...
			hasInputPins = true;
			OJAnnotatedOperation getInputPin = new OJAnnotatedOperation("get"+NameConverter.capitalize(inputPin.getName())+"InputPin");
			sb.append(getInputPin.getName() + "()");
			sb.append(",");
			getInputPin.setReturnType(TinkerBehaviorUtil.activityNodePathName(inputPin));
			getInputPin.getBody().addToStatements("return new " + TinkerBehaviorUtil.activityNodePathName(inputPin).getLast() + "(this.vertex.getOutEdges(\"inputPin" + NameConverter.capitalize(inputPin.getName() + "\").iterator().next().getInVertex(), this.contextObject)"));
...
//		getOutControlFlows.setReturnType(TinkerBehaviorUtil.tinkerActivityEdgePathName);
	...
	private void implementGetAction(OJAnnotatedClass inputPinClass, INakedPin oa) {
		...
		getAction.getBody().addToStatements("return new " + TinkerBehaviorUtil.activityNodePathName(oa.getAction()).getLast() + "(this.vertex.getInEdges(\"inputPin" + NameConverter.capitalize(oa.getName() + "\").iterator().next().getOutVertex(), this.contextObject)") );
