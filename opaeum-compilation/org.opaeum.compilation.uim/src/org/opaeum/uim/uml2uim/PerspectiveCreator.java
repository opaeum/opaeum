...
public class PerspectiveCreator extends AbstractUimSynchronizer2{
	...
	public void visitWorkspace(EmfWorkspace ws){
		URI uri = super.directoryUri.appendSegment("ui").appendFragment("perspective").appendFileExtension("uml");
	...
	public void visitOperation(Operation o){
		if(!UserInterfaceUtil.isUnderUserControl(explorerPosition)){
			ExplorerClassConstraint ecc = findOrCreateClassConstraint((Classifier) o.getOwner());
	...
	public void visitProperty(Property p){
		if(!UserInterfaceUtil.isUnderUserControl(explorerPosition)){
			ExplorerClassConstraint ecc = findOrCreateClassConstraint(EmfPropertyUtil.getOwningClassifier(p));
	...
	public void visitClassifier(Classifier c){
		if(!UserInterfaceUtil.isUnderUserControl(explorerPosition)){
			if(EmfClassifierUtil.isPersistentComplexStructure(c)){
