...
public class TupleTypeCreator extends DataTypeCreator {
	...
	public OJClass make(TupleType in, OJPackage tuplePack) {
		...
		TupleTypeMap TUPLE = new TupleTypeMap((TupleType) in);
	...
	private OJClass priv_make(TupleType in, OJPackage tuplePack) {
		...
		while (it.hasNext()) {
			...
			ClassifierMap mapper = new ClassifierMap((Classifier) decl.getType());
			created.addToFields(make_attribute(decl, mapper));
			constr.getBody().addToStatements(
					"this." + getFieldName(decl) + " = "
							+ getFieldName(decl));
			constr.addParam(getFieldName(decl),
					mapper.javaTypePath());
	 ...
	 * @return
	 */
	private OJOperation createGetter(Property decl,
			ClassifierMap mapper) {
	...
	private OJField make_attribute(Property decl,
			ClassifierMap mapper) {
	...
	/**
	 * Creates the operation 'equals' for <arg>in<\arg>. See ITEM 7 in Effective
	 * Java. The operation compares all non-derived attributes.
	 * 
	 * @param in
	 * @return
	 */
	...
	protected OJOperation equal_op(DataType in) {
		...
		/**
		 * <octel var="currentClass"> <method type="%JavaPathNames.Bool%"
		 * name="equals" var="oper"> <param type="%JavaPathNames.Object%"
		 * name="%param%"/> <body> <if> !(%param% instanceof %TYPE%) <then>
		 * return false; </then></if> %TYPE% par = (%TYPE%) %param%; </body>
		 * </method>
		 * 
		 * </octel>
		 */
