...
public class LoopExpCreator{
	...
	private String createCollectNested(IteratorExp exp,StringBuffer source,boolean isStatic,List<OJParameter> params){
		 ...
		 * implements %exp.toString()% <paramlist> <body> %resultType% result = %resultDefault%; <for
		 * name="%iterVarNames[0]%" type="%myListTypePath%"> <collection>%source.toString()%</collection> <body> %expType% bodyExpResult =
		 * %expStr%; if ( bodyExpResult != null ) result.add( bodyExpResult ); </body></for> return result; </body> </method> </octel>
		 */
	...
	private String createOne(IteratorExp exp,StringBuffer source,boolean isStatic,List<OJParameter> params){
		...
		/**
		 * <octel var="myClass"> <method type="%resultTypePath%" name="%operName%" visibility="%priv%" static="%isStatic%" var="oper"> <comment>
		 * implements %exp.toString()% <paramlist> <body> int _genNr = 0; <for name="%iterVarNames[0]%"
		 * type="%myListTypePath%"> <collection>%source.toString()%<if>
		 * <octel>
		 */
	...
	private String createIsUnique(IteratorExp exp,StringBuffer source,boolean isStatic,List<OJParameter> params){
		...
		/**
		 * <octel var="myClass"> <method type="%resultTypePath%" name="%operName%" visibility="%priv%" static="%isStatic%" var="oper"> <comment>
		 * implements %exp.toString()% <paramlist> <body> %valueType.getCollectionTypeName()% _valuesInSource =
		 * new %valueImplType.getCollectionTypeName()%(); <for name="%iterVarNames[0]%" type="%myListTypePath%">
		 * <collection>%source.toString()%<if>
		...
		OJSimpleStatement exp42 = new OJSimpleStatement(valueType.getCollectionTypeName() + " _valuesInSource = new "
	...
	private String createExists(IteratorExp exp,StringBuffer source,boolean isStatic,List<OJParameter> params){
		...
		/**
		 * <octel var="myClass"> <method type="%resultTypePath%" name="%operName%" visibility="%priv%" static="%isStatic%" var="oper"> <comment>
		 * implements %exp.toString()% <paramlist> <body> <for name="%iterVarNames[0]%"
		 * type="%myListTypePath%"> <collection>%source.toString()%<if>
		 * </body></for> return false; </body> </method> </octel>
		 */
	...
	private String createForAll(IteratorExp exp,StringBuffer source,boolean isStatic,List<OJParameter> params){
		...
		if(iterVarNames.length == 1){
			 ...
			 * <comment> implements %exp.toString()% <paramlist> <body> <for name="%iterVarNames[0]%"
			 * type="%myListTypePath%"> <collection>%source.toString()%<if>
			 ...
			 * <octel var="myClass"> <method type="%JavaPathNames.Bool%" name="%operName%" visibility="%priv%" static="%isStatic%" var="oper">
			 * <comment> implements %exp.toString()% <paramlist> <body> <for name="%iterVarNames[0]%"
			 * type="%myListTypePath%"> <collection>%source.toString()%</collection> <body> <for name="%iterVarNames[1]%" type="%myListTypePath%">
			 * <collection>%source.toString()%<for>
			 * return true; </body> </method> </octel>
			 */
