...
public class CumulativeDurationStrategyFactory extends AbstractStrategyFactory{
	public static class MyFormattingStrategy implements FormatterStrategy
	{
		...
		public void implementParse(OJAnnotatedOperation parse){
			parse.initializeResultVariable("value==null||value.length()==0?null: new CumulativeDuration(value)");
	...
	public static class MyJpaStrategy implements JpaStrategy{
		...
		public void annotate(OJAnnotatedClass c,OJAnnotatedField f,Property p){
			...
			overrides.addAnnotationValue(JpaUtil.createOverride("cumulativeQuantity", persistentName));
	...
	public CumulativeDurationStrategyFactory(){
	...
	public boolean appliesTo(DataType st){
		return st.getName().equals("CumulativeDuration");
	}
	@Override
	public String getRuntimeStrategyFactory(){
		return "org.opaeum.runtime.strategy.CumulativeDurationStrategyFactory";
	}
}
