		...
		public void pageClosed(IWorkbenchPage page){
			for(IEditorReference er:page.getEditorReferences()){
				if(er.getEditor(false) instanceof PapyrusMultiDiagramEditor){
					PapyrusMultiDiagramEditor e = (PapyrusMultiDiagramEditor) er.getEditor(false);
	...
	private void maybeSetCurrentEditContext(IWorkbenchPart part){
		...
		if(editor instanceof PapyrusMultiDiagramEditor){
			...
			if(uimFile.getParent().getName().equals("ui")){
				if(!UimContentAdapter.isListeningTo(e.getEditingDomain().getResourceSet())){
					e.getEditingDomain().getResourceSet().eAdapters().add(new UimContentAdapter());
				}
	...
	private void closeOrphanedUimModelers(IWorkbenchPage page){
		for(IEditorReference er:page.getEditorReferences()){
			if(er.getEditor(false) instanceof PapyrusMultiDiagramEditor){
				PapyrusMultiDiagramEditor e = (PapyrusMultiDiagramEditor) er.getEditor(false);
				if(e.getDiagram().getElement() instanceof UserInteractionElement){
					if(UmlUimLinks.getCurrentUmlLinks((UserInteractionElement) e.getDiagram().getElement()) == null){
						page.closeEditor(e, true);
					}
				}
			}
