...
public class UimViewProvider extends AbstractProvider implements IViewProvider{
	...
	protected boolean provides(CreateDiagramViewOperation op){
		return CubeQueryEditPart.MODEL_ID.equals(op.getSemanticHint())
	...
	protected boolean provides(CreateNodeViewOperation op){
		...
		if(op.getSemanticHint() == null){
			...
			if(elementType != null){
				...
				if(!CubeQueryEditPart.MODEL_ID.equals(UimVisualIDRegistry.getModelID(op.getContainerView()))){
	...
	public Diagram createDiagram(IAdaptable semanticAdapter,String diagramKind,PreferencesHint preferencesHint){
		...
		diagram.setType(CubeQueryEditPart.MODEL_ID);
	 ...
	 * @generated
	 */
	public Node createColumnAxisEntry_2001(EObject domainElement,View containerView,int index,boolean persisted,
			PreferencesHint preferencesHint){
		Shape node = NotationFactory.eINSTANCE.createShape();
		node.setLayoutConstraint(NotationFactory.eINSTANCE.createBounds());
		node.setType(UimVisualIDRegistry.getType(ColumnAxisEntryEditPart.VISUAL_ID));
		ViewUtil.insertChildView(containerView, node, index, persisted);
		node.setElement(domainElement);
		stampShortcut(containerView, node);
		// initializeFromPreferences 
		final IPreferenceStore prefStore = (IPreferenceStore) preferencesHint.getPreferenceStore();
		PreferenceInitializerForElementHelper.initForegroundFromPrefs(node, prefStore, "ColumnAxisEntry");
		PreferenceInitializerForElementHelper.initFontStyleFromPrefs(node, prefStore, "ColumnAxisEntry");
		PreferenceInitializerForElementHelper.initBackgroundFromPrefs(node, prefStore, "ColumnAxisEntry");
		return node;
	}
	/**
	 * @generated
	 */
	public Node createRowAxisEntry_2002(EObject domainElement,View containerView,int index,boolean persisted,PreferencesHint preferencesHint){
		Shape node = NotationFactory.eINSTANCE.createShape();
		node.setLayoutConstraint(NotationFactory.eINSTANCE.createBounds());
		node.setType(UimVisualIDRegistry.getType(RowAxisEntryEditPart.VISUAL_ID));
		ViewUtil.insertChildView(containerView, node, index, persisted);
		node.setElement(domainElement);
		stampShortcut(containerView, node);
		// initializeFromPreferences 
	...
	protected void stampShortcut(View containerView,Node target){
		if(!CubeQueryEditPart.MODEL_ID.equals(UimVisualIDRegistry.getModelID(containerView))){
			EAnnotation shortcutAnnotation = EcoreFactory.eINSTANCE.createEAnnotation();
			shortcutAnnotation.setSource("Shortcut"); //$NON-NLS-1$
			shortcutAnnotation.getDetails().put("modelID", CubeQueryEditPart.MODEL_ID); //$NON-NLS-1$
