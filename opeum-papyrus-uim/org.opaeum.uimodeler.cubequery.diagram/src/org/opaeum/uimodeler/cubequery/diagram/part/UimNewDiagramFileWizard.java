...
public class UimNewDiagramFileWizard extends Wizard{
	...
	public UimNewDiagramFileWizard(URI domainModelURI,EObject diagramRoot,TransactionalEditingDomain editingDomain){
		...
		myFileCreationPage.setDescription(NLS.bind(Messages.UimNewDiagramFileWizard_CreationPageDescription, CubeQueryEditPart.MODEL_ID));
		IPath filePath;
	...
	public boolean performFinish(){
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor,IAdaptable info) throws ExecutionException{
				...
				if(diagramVID != CubeQueryEditPart.VISUAL_ID){
					return CommandResult.newErrorCommandResult(Messages.UimNewDiagramFileWizard_IncorrectRootError);
				}
				Diagram diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(), CubeQueryEditPart.MODEL_ID,
		...
		try{
			...
			UimCubeQueryDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		}catch(PartInitException ex){
			UimCubeQueryDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
	/**
	...
	private static class DiagramRootElementSelectionPage extends ModelElementSelectionPage{
		...
		protected boolean validatePage(){
			...
			boolean result = ViewService.getInstance().provides(
					new CreateDiagramViewOperation(new EObjectAdapter(selectedModelElement), CubeQueryEditPart.MODEL_ID,
							UimCubeQueryDiagramEditorPlugin.DIAGRAM_PREFERENCES_HINT));
