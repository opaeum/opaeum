...
public class OperationButtonOperationSection extends OpaeumChooserPropertySection{
	...
	private Object[] getActionOperations(Collection<Operation> opers){
		List<Operation> results = new ArrayList<Operation>();
		for(Operation operation:opers){
			if(operation.getReturnResult() == null && !operation.isQuery() && !StereotypesHelper.hasStereotype(operation, "Responsibility")){
