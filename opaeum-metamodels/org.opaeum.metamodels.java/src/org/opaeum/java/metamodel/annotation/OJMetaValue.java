...
/**
 * A common superclass for annotation values and attribute values and defines all the value-declaration logic. In Java annotations can also
 * have a "value" if it has a single property name "value". As such, it is similar to an attribute value that could also have a value
 */
public abstract class OJMetaValue extends OJElement{
	...
	/**
	 * Utility method that generates the string representing the literal expression to assign to the value.
	 * 
	 * @param o
