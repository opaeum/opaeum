...
public class UserTextTableComposite extends Composite{
	...
	public void setOwner(LabelContainer owner){
		removeAdaptor();
		this.owner = owner;
		addAdaptor();
		refresh();
	...
	protected void createContents(Composite parent){
		...
		removeButton.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent event){
				...
				if(owner.getLabelOverride() != null){
					editingDomain.getCommandStack().execute(RemoveCommand.create(editingDomain, owner.getLabelOverride(), EcorePackage.eINSTANCE.getEAnnotation_Details(), object));
					refresh();
				}
	...
	protected void refresh(){
		...
		for(final Locale l:OpaeumEclipseContext.getContextFor(owner).getConfig().getSupportedLocales()){
			...
			menuItem.addSelectionListener(new SelectionListener(){
				...
				public void widgetSelected(SelectionEvent ev){
					String name=((UserInteractionElement) owner).getName();
					if(name==null || name.length()==0){
						name = owner.eClass().getName();
					}
					...
					if(owner.getLabelOverride()==null){
						SetCommand set = (SetCommand) SetCommand.create(editingDomain, owner, UimPackage.eINSTANCE.getLabelContainer_LabelOverride(), UimFactory.eINSTANCE.createLabels());
						editingDomain.getCommandStack().execute(set);
					}
					AddCommand addCommand = (AddCommand) AddCommand.create(editingDomain, owner.getLabelOverride(), EcorePackage.eINSTANCE.getEAnnotation_Details(), entry);
		...
		if(!userTextTable.isDisposed()){
			...
			for(MenuItem menuItem:items){
				menuItem.setEnabled(owner.getLabelOverride() == null ||  !owner.getLabelOverride().getDetails().containsKey(((Locale) menuItem.getData()).toString()));
			...
			if(owner.getLabelOverride() == null){
				...
				userTextTableViewer.setInput(owner.getLabelOverride().getDetails().entrySet());
			}
		}
		isRefreshing = false;
	}
