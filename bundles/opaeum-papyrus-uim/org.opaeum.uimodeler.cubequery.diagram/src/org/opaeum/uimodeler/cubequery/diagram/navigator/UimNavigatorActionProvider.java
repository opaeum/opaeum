...
import org.opaeum.uimodeler.cubequery.diagram.part.Messages;
 ...
 * @generated
 */
public class UimNavigatorActionProvider extends CommonActionProvider{
	/**
	 * @generated
	 */
	...
	private static class OpenDiagramAction extends Action{
		...
		public final void selectionChanged(IStructuredSelection selection){
			...
			if(selection.size() == 1){
				...
				if(selectedElement instanceof UimNavigatorItem){
					selectedElement = ((UimNavigatorItem) selectedElement).getView();
				}else if(selectedElement instanceof IAdaptable){
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if(selectedElement instanceof Diagram){
					Diagram diagram = (Diagram) selectedElement;
					if(CubeQueryEditPart.MODEL_ID.equals(UimVisualIDRegistry.getModelID(diagram))){
						myDiagram = diagram;
					}
				}
			}
		...
		public void run(){
			...
			try{
				...
				UimCubeQueryDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
		/**
		 * @generated
