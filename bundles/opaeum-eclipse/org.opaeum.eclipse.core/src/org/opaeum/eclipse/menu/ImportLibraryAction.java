...
public class ImportLibraryAction extends AbstractOpaeumAction{
	...
	public void run(){
		...
		if(!model.getImportedPackages().contains(library)){
			currentContext.getEditingDomain().getCommandStack(). execute(AddCommand.create(currentContext.getEditingDomain(), model, UMLPackage.eINSTANCE.getNamespace_PackageImport(), pi));
		}
	}
}
