...
public class StdLibCreator{
	...
	public OJClass makeStdLib(OJPackage utilPack){
		...
		stdlibCls = new OJClass(stdLibClassName);
	...
	private void addInExcludingOpers(){
		...
		if(!(StdlibMap.javaOrderedSetType.equals(StdlibMap.javaSetType) || StdlibMap.javaOrderedSetType.equals(StdlibMap.javaBagType) || StdlibMap.javaOrderedSetType
				.equals(StdlibMap.javaSequenceType))){
	...
	private OJOperation makeSetEquals(){
		...
		/**
		 * <octel var="stdlibCls"> <method type="%JavaPathNames.Bool%" name="%operName%" visibility="%OJVisibilityKind.PUBLIC%" static="%true%"
		 * var="oper"> <comment> implements the equals operation for sets </comment> <param type="%type%" name="source"/> <param type="%type%"
		 * name="arg"/> <body> <if> source.size() != arg.size() <then> return false; </then></if> Iterator it = arg.iterator(); <while>
		 * it.hasNext() <body> Object elem = it.next(); <if> !source.contains(elem) <then> return false; </then></if> </body></while> return
	...
	private OJOperation makeSequenceEquals(){
		...
		/**
		 * <octel var="stdlibCls"> <method type="%JavaPathNames.Bool%" name="%operName%" visibility="%OJVisibilityKind.PUBLIC%" static="%true%"
		 * var="oper"> <comment> implements the equals operation for sequences </comment> <param type="%type%" name="source"/> <param
		 * type="%type%" name="arg"/> <body var="body"> <if> source.size() != arg.size() <then> return false; </then></if> Iterator it1 =
		 * source.iterator(); Iterator it2 = arg.iterator(); </body> </method> </octel>
		 */
		/* <method> */
		oper = new OJOperation(operName);
		stdlibCls.addToOperations(oper);
	...
	private OJOperation makeBagEquals(){
		...
		/**
		 * <octel var="stdlibCls"> <import> %implType% </import>
		 * 
		 * <method genparam="%genericTypeParam%" type="%JavaPathNames.Bool%" name="%operName%" visibility="%OJVisibilityKind.PUBLIC%"
		 * static="%true%" var="oper"> <param type="%type%" name="source"/> <param type="%type%" name="arg"/> <comment> implements the equals
		 * operation for bags </comment> <body var="body"> <comment> make copy of arguments in order to manipulate the collection </comment>
		 * <if> source.size() != arg.size() <then> return false; </then></if> %type.getCollectionTypeName()% myArg = new
		 * %implType.getCollectionTypeName()%( arg ); <for name="elem" type="%genericTypeParam%"> <collection>source</collection> <body>
		 * <if>myArg.contains(elem)<then> myArg.remove(elem); </then><else> return false; </else></if> </body></for> return myArg.isEmpty();
		 * </body> </method> </octel>
		 */
		/* <import> */
		...
		/* <then/> */
		/* <if/> */
		OJSimpleStatement exp14 = new OJSimpleStatement(type.getTypeNameWithTypeArguments() + " myArg = new "
				+ implType.getTypeNameWithTypeArguments() + "( arg )");
		...
		/* <collection> */
		for1.setCollectionExpression("source");
		/* <collection/> */
	...
	private OJOperation makeOrderedSetEquals(){
		...
		/**
		 * <octel var="stdlibCls"> <method type="%JavaPathNames.Bool%" name="%operName%" visibility="%OJVisibilityKind.PUBLIC%" static="%true%"
		 * var="oper"> <comment> implements the equals operation for orderedsets </comment> <param type="%type%" name="source"/> <param
		 * type="%type%" name="arg"/> <body var="body"> <if> source.size() != arg.size() <then> return false; </then></if> Iterator it1 =
		 * source.iterator(); Iterator it2 = arg.iterator(); </body> </method> </octel>
		 */
		/* <method> */
	...
	private void makeEqualsCompare(OJBlock body){
		/**
		 * <octel var="body"> <while> it1.hasNext() <body> Object elem1 = it1.next(); Object elem2 = it2.next(); <if> elem1 instanceof Integer
		 * <then> <if> ((Integer)elem1).intValue() != ((Integer)elem2).intValue() <then> return false; </then></if> </then><else> <if> elem1
		 * instanceof Float <then> <if> ((Float)elem1).floatValue() != ((Float)elem2).floatValue() <then> return false; </then></if>
		 * </then><else> <if> elem1 instanceof Boolean <then> <if> ((Boolean)elem1).booleanValue() != ((Boolean)elem2).booleanValue() <then>
		 * return false; </then></if> </then><else> <if> !elem1.equals(elem2) <then> return false; </then></if> </else></if> </else></if>
		 * </else></if> </body></while> </octel>
		 */
		/* <while> */
	...
	private void makeFlatten(OJPathName returnType,String myDefault,String operName,String comment){
		...
		OJOperation method1 = new OJOperation(operName);
	...
	private void makeCollAsXOper(String paramName,String operName,OJPathName elementType,OJPathName myType,OJPathName myImplType){
		...
		OJOperation method2 = new OJOperation(operName);
	...
	private void makeAsCollectionOper(String paramName,String operName,OJPathName elementType,OJPathName myType,OJPathName myImplType){
		OJPathName RESULT_TYPE = myType.getCopy();
		OJPathName IMPL_TYPE = myImplType.getCopy();
		if(elementType == StdlibMap.javaCollectionType){
			 ...
			 * <octel var="stdlibCls">
			 * 
			 * <method static="%true%" type="%RESULT_TYPE%" name ="%operName%"> <param type="%elementType%" name="%paramName%"/> <body>
			 * %RESULT_TYPE.getCollectionTypeName()% result = %INIT_VALUE%; <if>%paramName% != null<then> result.addAll( %paramName% );
			 * </then></if> return result; </body></method>
			 * 
			 * </octel>
			 */
			/* <method> */
			OJOperation method3 = new OJOperation(operName);
			 ...
			 * 
			 * <method static="%true%" type="%RESULT_TYPE%" name ="%operName%"> <param type="%elementType%" name="%paramName%"/> <body>
			 * %RESULT_TYPE.getCollectionTypeName()% result = %INIT_VALUE%; <if octel_condition="%nonPrim%">%paramName% != null<then> result.add(
			 * %elemToAdd% ); </then></if> <exp octel_condition="%!nonPrim%"> result.add( %elemToAdd% )</exp> return result; </body></method>
			 * 
			 * </octel>
			 */
			/* <method> */
			OJOperation method4 = new OJOperation(operName);
	...
	private void makeIncludingX(OJPathName myType,OJPathName myImplType){
		 ...
		 * <octel var="stdlibCls">
		 * 
		 * <method genparam="%genericTypeParam%" static="%true%" type="%RESULT_TYPE%" name ="%operName%"> <param type="%RESULT_TYPE%"
		 * name="mySource"/> <param type="%genericTypeParam%" name="myElem"/> <body> %RESULT_TYPE.getCollectionTypeName()% result =
		 * %INIT_VALUE%; <if>myElem != null<then> result.add(myElem); </then></if> return result; </body></method>
		 * 
		 * </octel>
		 */
		/* <method> */
		OJOperation method5 = new OJOperation(operName);
	...
	private void makeExcludingX(OJPathName myType,OJPathName myImplType){
		 ...
		 * <method genparam="%genericTypeParam%" static="%true%" type="%RESULT_TYPE%" name ="%operName%"> <param type="%RESULT_TYPE%"
		 * name="mySource"/> <param type="%genericTypeParam%" name="myElem"/> <body> %RESULT_TYPE.getCollectionTypeName()% result =
		 * %INIT_VALUE%; <if>myElem != null<then> result.remove(myElem); </then></if> return result; </body></method>
		 * 
		 * </octel>
		 */
		/* <method> */
		OJOperation method6 = new OJOperation(operName);
	...
	private void makeIntersection(){
		 ...
		 * <octel var="stdlibCls">
		 * 
		 * <method genparam="%genericTypeParam%" static="%true%" type="%RESULT_TYPE%" name ="%operName%"> <param type="%PARAM_TYPE%"
		 * name="mySource"/> <param type="%PARAM_TYPE%" name="myElem"/> <body> %RESULT_TYPE.getCollectionTypeName()% result = %INIT_VALUE%;
		 * <if>myElem != null<then> result.retainAll(myElem); </then></if> return result; </body></method>
		 * 
		 * </octel>
		 */
		/* <method> */
		OJOperation method7 = new OJOperation(operName);
	...
	private void makeInsertAt(OJPathName myType){
		...
		OJPathName genericTypeParam = new OJPathName("T");
		RESULT_TYPE.addToElementTypes(genericTypeParam);
		OJOperation method8 = new OJOperation(operName);
