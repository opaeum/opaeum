...
public class TinkerAuditSchemaGenerator extends AbstractJavaProducingVisitor {
	...
	public void visitClass(INakedEntity c) {
		if (OJUtil.hasOJClass(c) && !(c instanceof INakedSimpleType) && !c.getIsAbstract()) {
			...
			ifNotHasSchema.addToThenPart("rawGraph.getMetadata().getSchema().createClass(\"" + c.getMappingInfo().getJavaName() + TinkerAuditCreator.AUDIT
			...
			getClassNames.getBody().getStatements().add(getClassNames.getBody().getStatements().size() - 1, new OJSimpleStatement("result.add(\"" + c.getMappingInfo().getJavaName() + TinkerAuditCreator.AUDIT  + "\")"));
		}
	}
