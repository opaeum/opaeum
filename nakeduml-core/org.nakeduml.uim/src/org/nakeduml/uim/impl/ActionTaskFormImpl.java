...
public class ActionTaskFormImpl extends FormPanelImpl implements ActionTaskForm {
		...
		if (folder != null && folder.eIsProxy()) {
			...
			if (folder != oldFolder) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, UimPackage.ACTION_TASK_FORM__FOLDER, oldFolder, folder));
			}
		}
		return folder;
	}
	...
	public void setFolder(ActivityFolder newFolder) {
		...
		folder = newFolder;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, UimPackage.ACTION_TASK_FORM__FOLDER, oldFolder, folder));
	}
