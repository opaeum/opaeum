...
public class ClassFormImpl extends FormPanelImpl implements ClassForm {
		...
		if (folder != null && folder.eIsProxy()) {
			...
			if (folder != oldFolder) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, UimPackage.CLASS_FORM__FOLDER, oldFolder, folder));
			}
		}
		return folder;
	}
	...
	public void setFolder(OperationContainingFolder newFolder) {
		...
		folder = newFolder;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, UimPackage.CLASS_FORM__FOLDER, oldFolder, folder));
	}
