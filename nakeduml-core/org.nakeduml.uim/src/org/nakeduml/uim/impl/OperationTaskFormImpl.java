...
public class OperationTaskFormImpl extends FormPanelImpl implements OperationTaskForm {
		...
		if (folder != null && folder.eIsProxy()) {
			...
			if (folder != oldFolder) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, UimPackage.OPERATION_TASK_FORM__FOLDER, oldFolder, folder));
			}
		}
		return folder;
	}
	...
	public void setFolder(EntityFolder newFolder) {
		...
		folder = newFolder;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, UimPackage.OPERATION_TASK_FORM__FOLDER, oldFolder, folder));
	}
