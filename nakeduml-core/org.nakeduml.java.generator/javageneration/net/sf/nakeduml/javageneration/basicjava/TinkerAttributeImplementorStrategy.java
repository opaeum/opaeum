...
public class TinkerAttributeImplementorStrategy implements AttributeImplementorStrategy {
	...
	private void buildPolymorphicGetterForToOne(NakedStructuralFeatureMap map, OJOperation getter, Collection<INakedRootObject> modelInScope, boolean oneToOne) {
		...
		for (INakedClassifier subClassClassifier : subClasses) {
			for (INakedClassifier otherSubClassClassifier : otherSubClasses) {
				...
				getter.getBody().addToStatements(
						new OJIfStatement("iter" + count + ".iterator().hasNext()", "return new "
								+ subClassClassifier.getMappingInfo().getJavaName().toString() + "(iter" + count + ".iterator().next()."
	...
	private void createPolymorphicToOneRelationship(INakedClassifier umlOwner, NakedStructuralFeatureMap map, NakedStructuralFeatureMap otherMap, OJOperation setter,
			Collection<INakedRootObject> modelInScope, boolean oneToOne) {
		boolean isComposite = map.getProperty().isComposite();
		isComposite = calculateNonCompositeDirection(map, oneToOne, isComposite);
			...
			for (INakedClassifier otherSubClassClassifier : otherSubClasses) {
				...
				ifParamNotNull.getThenPart().addToStatements(
						new OJIfStatement("(this.getClass() == " + subClassClassifier.getMappingInfo().getJavaName().toString() + ".class) && ("
								+ map.umlName() + ".getClass() == " + otherSubClassClassifier.getMappingInfo().getJavaName().toString() + ".class)", UtilityCreator
								.getUtilPathName().toJavaString()
								+ ".DbThreadVar.getDB().addEdge(null, "
								+ (!isComposite ? "((" + otherSubClassClassifier.getMappingInfo().getJavaName().toString() + ")" + map.umlName() + ").getVertex(), this.vertex," : "this.vertex, ((" + otherSubClassClassifier.getMappingInfo().getJavaName().toString() + ")" + map.umlName() + ").getVertex(),")
								+ "\""
								+ relationshipName + "\")"));
			}
		}
