...
public abstract class AbstractBooleanOnStereotypeSection extends AbstractTabbedPropertySection{
	...
	public void refresh(){
		...
		if(getElement().isStereotypeApplied(stereotype)){
			Boolean value = (Boolean) getElement().getValue(stereotype, getAttributeName());
			check.setSelection(value == null ? getDefaultValue() : value);
		}else{
			check.setSelection(getDefaultValue());
		}
	}
	@Override
	...
	protected void createWidgets(Composite composite){
		...
		check.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent e){
				...
				Command cmd = SetCommand.create(getEditingDomain(), getElement().getStereotypeApplication(stereotype), feature, check.getSelection());
				getEditingDomain().getCommandStack().execute(cmd);
			}
			@Override
