...
public abstract class ActionActivation<AE extends IActivityNodeContainerExecution, T extends ActivityToken<AE>>  extends ActivityNodeActivation<AE,T>{
	...
	public void sendOffers(){
		...
		if(this.outgoingEdges.size() == 1){
			...
			T parent = (T) group.createToken(TokenKind.CONTROl);
			tokens.add(parent);
			...
			for(ActivityEdgeInstance<AE,T> edge:this.outgoingEdges){
				List<T> tokens = new ArrayList<T>();
				T forkedToken = (T) group.createToken(TokenKind.FORKED);
	...
	public void putToken(String pin,Object value){
		...
		T token = (T) group.createToken(TokenKind.OBJECT);
