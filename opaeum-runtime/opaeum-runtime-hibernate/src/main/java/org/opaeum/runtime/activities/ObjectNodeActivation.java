...
public abstract class ObjectNodeActivation <AE extends IActivityNodeContainerExecution, T extends ActivityToken<AE>>  extends ActivityNodeActivation<AE,T>{
	...
	public void sendOffers(List<T> tokens){
		...
		if(tokens.size() == 0){
			tokens.add((T) group.createToken(TokenKind.OBJECT));
		}
