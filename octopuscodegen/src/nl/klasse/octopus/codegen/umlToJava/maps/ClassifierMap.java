...
/**
 * ClassifierMap : This class holds all the information on how a UML classifier is transformed
 * into the corresponding Java class.
 * 
 * It is able to give information on the following items.
 * 		- the path in the java model of the corresponding Java class/type,
 * 			e.g. for UML classifier<code>novels::Book</code>the result is<code>novels.Book</code>,
 * 				or - case of a facade -<code>novels.internal.Book</code>;
 * 			e.g. for UML classifier <code>Integer</code> the result is<code>java.util.int</code>;
 * 			e.g. for UML classifier <code>Set(novels::Book)</code> the result is<code>java.util.Set<novels.Book></code>,
 * 				or - case of a facade -<code>java.util.Set<novels.internal.Book></code>;
 * 		- the path in the java model of the facade interface,
 * 			e.g. for UML classifier<code>novels::Book</code>the result is<code>novels.IBook</code>;
 * 			e.g. for UML classifier <code>Integer</code> the result is<code>java.util.int</code>;
 * 			e.g. for UML classifier <code>Set(novels::Book)</code> the result is<code>java.util.Set<novels.IBook></code>;
 * 		- the path in the java model of the Java type that is used to give an element of this UML type
 *			an initial value,
 * 			e.g. for UML classifier<code>novels::Book</code>the result is<code>novels.Book</code>;
 * 			e.g. for UML classifier <code>Integer</code> the result is<code>java.util.int</code>;
 * 			e.g. for UML classifier <code>Set(novels::Book)</code> the result is<code>java.util.HashSet<novels.Book></code>;
 * 		- a string that is used to give an element of this UML type an initial value,
 * 			e.g. for UML classifier<code>novels::Book</code>the result is<code>new Book()</code>;
 * 			e.g. for UML classifier <code>Integer</code> the result is<code>0</code>;
 * 			e.g. for UML classifier <code>Set(novels::Book)</code> the result is<code>new HashSet<novels.Book>()</code>;
 * 
 ...
 * 
 * If the UML classifier is a primitive type, then this class is also able to return the Java object type for that primitive type. 
 * E.g. the Java object type for the UML type <code>Integer</code> is <code>java.util.Object</code>.
 * 
 */
public class ClassifierMap extends PackageableElementMap {
	...
	public ClassifierMap(IClassifier modelClass) {
		...
		if (this.modelClass.isCollectionKind()) {
			...
			elementMap = new ClassifierMap(type.getElementType());
		}
	}
	...
	public boolean isNestedCollection(){
		return 	modelClass.isCollectionKind() &&
				( new ClassifierMap(((ICollectionType)modelClass).getElementType()).isCollection() );
	}
	...
	/** If this classifier has a facade interface, this method returns
	 * the path of the java type of that facade interface. If the classifier
	 * does not have a facade interface, the path of the java type for the 
	 * classifier is returned, i.e. this method gives the same result as 
	 * this.javaTypePath().
	 * 
	 * @return
	 */
	...
	/** The java type of the default value for this class.
	 * Note that this type may be different form the java
	 * type for this class. E.g. the java type for the default 
	 * value of an OCL sequence is <code>ArrayList</code>,
	 * whereas the java type is <code>List</code>.
	 * 
	 * @return
	 */
	...
	/** If this classifier represents a collection, this
	 * method returns the java type of the elements in the
	 * collection. E.g. the java element type of Sequence{1,2,3}
	 * is <code>java.util.int</code>.
	 * 
	 * @return
	 */
	...
	/** If this classifier represents a collection and it has a facade interface,
	 * this method returns the path of the java type of the facade
	 * of the elements in the collection. E.g. the java element facade type of a
	 * Sequence of <code>pack1.SomeClass</code> objects is <code>pack1.ISomeClass</code>.
	 * 
	 * @return
	 */
	...
	/** If this classifier represents a collection, this method returns 
	 * the path of the java type of the default initial value of the elements in the
	 * collection. E.g. the java element type of Sequence{Set{1,2,3}}
	 * is <code>HashSet</code>.
	 * 
	 * @return
	 */
	...
	/** If this classifier represents a collection, this
	 * method returns the java default initial value of the elements in the
	 * collection. E.g. the java default initial value of Sequence{1,2,3}
	 * is <code>0</code>.
	 * 
	 * @return
	 */
	...
	/** If this classifier has a facade interface, this method returns
	 * the default initial value of that facade interface. If the classifier
	 * does not have a facade interface, the default initial value for the 
	...
	/** If this classifier represents a collection, this method returns the name of the method
	 * that copies an instance of the collection into an unmodifiable instance. E.g.
	 * for a Sequence this method returns <code>Collections.unmodifiableList</code>.
	 * @return
	 */
	...
	private String getJavaDefault(IDataType t){
		String result = "null";
		if (t.getName().equals(IOclLibrary.OclAnyTypeName)) result = "null";
		if (t.getName().equals(IOclLibrary.OclTypeTypeName)) result = "null";
		if (t.getName().equals(IOclLibrary.OclVoidTypeName)) result = "null";
		return result;
	}	
	...
	private String getJavaDefault(IPrimitiveType t){
		...
		if (t.getName().equals(IOclLibrary.StringTypeName)) result = "\"\"";
		if (t.getName().equals(IOclLibrary.RealTypeName)) result = "0";
		if (t.getName().equals(IOclLibrary.IntegerTypeName)) result = "0";
		if (t.getName().equals(IOclLibrary.BooleanTypeName)) result = "false";
		return result;
	}	
...
//
	...
	private void addElementType(ICollectionType t, OJPathName path) {
		IClassifier elemType = t.getElementType();
		...
		if ( elemType instanceof StdlibPrimitiveType ) {
			...
			ClassifierMap innerMap = new ClassifierMap(t.getElementType());
			innerPath = innerMap.javaFacadeTypePath(); // if no facade present, this will give the same result as javaTypePath()
		}
		if (innerPath != null) path.addToElementTypes(innerPath);
	}

	private OJPathName pathname(IModelElement t) {
	...
	/** The Java class name without any prefix, but with element types if the UML classifier is a collection.
	 *   E.g. Set<Book>, Chapter.
	 * 
	 * @return String
	 */
	...
	/** If this classifier has a facade interface, this method returns
	 * the name of the java type of that facade interface. If the classifier
	 * does not have a facade interface, the name of the java type for the 
	 * classifier is returned.
	 * 
	 * @return
	 */
	...
	/** If this classifier represents a collection, this method returns 
	 * the name of the java type of the default initial value of the elements in the
	 * collection. E.g. the java element type of Sequence{Set{1,2,3}}
	 * is <code>HashSet</code>.
	 * 
	 * @return
