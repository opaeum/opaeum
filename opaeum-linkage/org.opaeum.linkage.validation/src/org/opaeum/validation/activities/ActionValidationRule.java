...
public enum ActionValidationRule implements IValidationRule{
	...
	CALL_BEHAVIOR_ACTION_REQUIRES_BEHAVIOR("Call Behavior actions must have their Behavior specified",
			"No Method, Process or Screenflow  specified for {0}"),
	...
	CALL_OPERATION_ACTION_TARGET_TYPE_INVALID("Call Operation's effective target type must conform to the type declaring the operation",
			"The type of {0} does not conform to the {1} which declares the invoked operation {2}"),
	CALL_OPERATION_ACTION_REQUIRES_OPERATION("Call Operation actions must have their Behavior specified",
			"No Operation or Reponsibility specified for {0}"),
	FEATURE_ACTION_REQUIRES_FEATURE("Structural Feature actions must have their Structural Feature specified",
			"No Structural Feature specified for {0}"),
	...
	EXPANSION_NODE_REQUIRES_EXPANSION_REGION(
			"Expansion nodes must be specified as either input elements or output elements of an Expansion Region",
			"{0} is not associated with an Expansion Region as either input element or output element"),
	ACCEPT_CALL_RETURN_INFO_MUST_LINK(
			"AcceptCallActions must have a returnInfo OutputPin that links up to the returnInfo InputPin of a ReplyAction",
			"{0} does not have a returnInfo output pin that links up with the returnInfo input pin of an subsequent ReplyAction"),
	REPLY_ACTION_RETURN_INFO_MUST_LINK(
			"Reply actions must have a returnInfo InputPin that links up to the returnInfo OutputPin of a preceding AcceptCallAction",
