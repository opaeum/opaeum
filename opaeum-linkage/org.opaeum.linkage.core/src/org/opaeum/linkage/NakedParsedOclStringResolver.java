...
public class NakedParsedOclStringResolver extends AbstractModelElementLinker {
	...
	protected int getThreadPoolSize() {
		return 1;// FOr some reason this is required TODO investigate why
					// because it takes a while to execute
	}
		...
		if (edge.hasGuard() && edge.getGuard().getValue() instanceof ParsedOclString) {
			...
			if (owningBehavior instanceof INakedActivity) {
				...
				if (edge instanceof INakedObjectFlow) {
					environmentFactory.addObjectTokenParameter(env, (INakedObjectFlow) edge);
				}
			} else {
	...
	private void replacePreAndBodyConditions(INakedOperation op, INakedClassifier owner, Environment env) {
		...
		if (op.getBodyCondition() != null && op.getBodyCondition().getSpecification() != null && op.getBodyCondition().getSpecification().getValue() instanceof ParsedOclString) {
			...
			if (bodyCondition.isValidOclValue() && op.getReturnParameter() != null) {
				// overridePinType(op.getReturnParameter(),
				// ocl.getExpression().getExpressionType());
			}
		}
	}
	...
	public void visitAction(INakedAction a) {
		// NB!!! vistAfter to ensure valuepins are done first so that their
	...
	private IOclContext replaceSingleParsedOclString(ParsedOclString holder, INakedClassifier c, IClassifier expectedType, Environment env, boolean ignoreMultiplicity) {
		if (holder.getExpressionString() == null) {
			this.getErrorMap().putError((INakedElement) holder.getOwningModelElement().getModelElement(), CoreValidationRule.OCL, EmfValidationUtil.OCL_EXPRESSION_REQUIRED);
			...
			try {
				...
				if (expectedType != null) {
					if (expectedType instanceof StdlibCollectionType && !(expressionType instanceof StdlibCollectionType)) {
						...
						// be lenient with multiplicity - will likely be
					...
					if (!expressionType.conformsTo(expectedType)) {
						this.getErrorMap().putError(
								...
								"Return value of type " + expectedType == null ? "null" : expectedType.getName() + " expected, but the expression returns a value of type "
