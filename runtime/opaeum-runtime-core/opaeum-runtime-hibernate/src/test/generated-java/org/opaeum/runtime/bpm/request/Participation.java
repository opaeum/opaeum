...
@NumlMetaInfo(applicationIdentifier="opaeum_hibernate_tests",uuid="252060@_jRjnII6MEeCrtavWRHwoHg")
@Proxy(lazy=false)
...
public class Participation implements IPersistentObject, IEventGenerator, HibernateEntity, CompositionNode, Serializable {
	...
	public void setParticipant(IParticipant participant) {
		if ( this.getParticipant()!=null ) {
			this.getParticipant().z_internalRemoveFromParticipationParticipant_participation(getParticipationParticipant_participant());
		}
		if ( participant == null ) {
			this.z_internalRemoveFromParticipationParticipant_participant(this.getParticipationParticipant_participant());
		} else {
			ParticipationParticipant participationParticipant_participant = new ParticipationParticipant((Participation)this,(IParticipant)participant);
			this.z_internalAddToParticipationParticipant_participant(participationParticipant_participant);
			participant.z_internalAddToParticipationParticipant_participation(participationParticipant_participant);
		}
	}
