...
@NumlMetaInfo(applicationIdentifier="opaeum_hibernate_tests",uuid="252060@_k23OoEtmEeGd4cpyhpib9Q")
...
public class PersonNode implements IPersonNode, IPersistentObject, IEventGenerator, HibernateEntity, CompositionNode, Serializable {
	...
	public void addToBusinessActor(IBusinessActor businessActor) {
		if ( businessActor!=null && !this.getBusinessActor().contains(businessActor) ) {
			PersonFullfillsActorRole newLink = new PersonFullfillsActorRole((PersonNode)this,(IBusinessActor)businessActor);
			if ( businessActor.getRepresentedPerson()!=null ) {
				businessActor.getRepresentedPerson().removeFromBusinessActor(businessActor);
			}
			this.z_internalAddToPersonFullfillsActorRole_businessActor(newLink);
			businessActor.z_internalAddToPersonFullfillsActorRole_representedPerson(newLink);
		}
	}
	
	public void addToBusinessRole(IBusinessRole businessRole) {
		if ( businessRole!=null && !this.getBusinessRole().contains(businessRole) ) {
			PersonInBusinessRole newLink = new PersonInBusinessRole((PersonNode)this,(IBusinessRole)businessRole);
			if ( businessRole.getRepresentedPerson()!=null ) {
				businessRole.getRepresentedPerson().removeFromBusinessRole(businessRole);
			}
			this.z_internalAddToPersonInBusinessRole_businessRole(newLink);
			businessRole.z_internalAddToPersonInBusinessRole_representedPerson(newLink);
		}
	}
	...
	public void addToEMailAddress(PersonEMailAddressType type, PersonEMailAddress eMailAddress) {
		if ( eMailAddress!=null ) {
			if ( eMailAddress.getPerson()!=null ) {
				eMailAddress.getPerson().removeFromEMailAddress(eMailAddress.getType(),eMailAddress);
			}
	...
	public void addToLeave(Leave leave) {
		if ( leave!=null ) {
			if ( leave.getPerson()!=null ) {
				leave.getPerson().removeFromLeave(leave);
			}
	...
	public void addToOwningObject() {
		if ( getUsername()==null ) {
			throw new IllegalStateException("The qualifying property 'username' must be set before adding a value to 'businessNetwork'");
		}
	...
	public void addToPhoneNumber(PersonPhoneNumberType type, PersonPhoneNumber phoneNumber) {
		if ( phoneNumber!=null ) {
			if ( phoneNumber.getPerson()!=null ) {
				phoneNumber.getPerson().removeFromPhoneNumber(phoneNumber.getType(),phoneNumber);
			}
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("phoneNumber") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("213312905486829476")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					...
					if ( currentPropertyValueNode instanceof Element ) {
						...
						curVal.z_internalAddToPerson(this);
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("leave") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("8728994280524309614")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					...
					if ( currentPropertyValueNode instanceof Element ) {
						...
						curVal.buildTreeFromXml((Element)currentPropertyValueNode,map);
						this.z_internalAddToLeave(curVal);
						curVal.z_internalAddToPerson(this);
						map.put(curVal.getUid(), curVal);
					}
				}
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("postalAddress") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("3364558357702710040")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					...
					if ( currentPropertyValueNode instanceof Element ) {
						...
						curVal.buildTreeFromXml((Element)currentPropertyValueNode,map);
						this.z_internalAddToPostalAddress(curVal);
						curVal.z_internalAddToPerson(this);
						map.put(curVal.getUid(), curVal);
					}
				}
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("physicalAddress") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("3105719662914651808")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					...
					if ( currentPropertyValueNode instanceof Element ) {
						...
						curVal.buildTreeFromXml((Element)currentPropertyValueNode,map);
						this.z_internalAddToPhysicalAddress(curVal);
						curVal.z_internalAddToPerson(this);
						map.put(curVal.getUid(), curVal);
					}
				}
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("personFullfillsActorRole_businessActor") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("8013149829678573783")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					...
					if ( currentPropertyValueNode instanceof Element ) {
						...
						curVal.buildTreeFromXml((Element)currentPropertyValueNode,map);
						this.z_internalAddToPersonFullfillsActorRole_businessActor(curVal);
						curVal.z_internalAddToRepresentedPerson(this);
						map.put(curVal.getUid(), curVal);
					}
				}
			}
	...
	public void copyShallowState(PersonNode from, PersonNode to) {
		...
		if ( from.getPostalAddress()!=null ) {
			to.z_internalAddToPostalAddress(from.getPostalAddress().makeShallowCopy());
		}
		if ( from.getPhysicalAddress()!=null ) {
			to.z_internalAddToPhysicalAddress(from.getPhysicalAddress().makeShallowCopy());
		}
	...
	public void copyState(PersonNode from, PersonNode to) {
		...
		to.setUsername(from.getUsername());
		if ( from.getPostalAddress()!=null ) {
			to.z_internalAddToPostalAddress(from.getPostalAddress().makeCopy());
		}
		if ( from.getPhysicalAddress()!=null ) {
			to.z_internalAddToPhysicalAddress(from.getPhysicalAddress().makeCopy());
		}
		to.setRefreshToken(from.getRefreshToken());
		to.setTokenExpiryDateTime(from.getTokenExpiryDateTime());
		to.setPreferredLocale(from.getPreferredLocale());
	...
	public void markDeleted() {
		...
		for ( PersonInBusinessRole link : new HashSet<PersonInBusinessRole>(getPersonInBusinessRole_businessRole()) ) {
			link.getBusinessRole().z_internalRemoveFromPersonInBusinessRole_representedPerson(link);
		}
		for ( PersonFullfillsActorRole link : new HashSet<PersonFullfillsActorRole>(getPersonFullfillsActorRole_businessActor()) ) {
			link.getBusinessActor().z_internalRemoveFromPersonFullfillsActorRole_representedPerson(link);
		}
		...
		while ( i<propertyNodes.getLength() ) {
			...
			if ( currentPropertyNode instanceof Element && (currentPropertyNode.getNodeName().equals("personInBusinessRole_businessRole") || ((Element)currentPropertyNode).getAttribute("propertyId").equals("7482106327188733314")) ) {
				...
				while ( j<propertyValueNodes.getLength() ) {
					Node currentPropertyValueNode = propertyValueNodes.item(j++);
					if ( currentPropertyValueNode instanceof Element ) {
						PersonInBusinessRole personInBusinessRole_businessRole = (PersonInBusinessRole)map.get(((Element)currentPropertyValueNode).getAttribute("uid"));
	...
	public void removeAllFromBusinessRole(Set<? extends IBusinessRole> businessRole) {
	...
	public void removeAllFromEMailAddress(Set<? extends PersonEMailAddress> eMailAddress) {
	...
	public void removeAllFromLeave(Set<? extends Leave> leave) {
	...
	public void removeAllFromPhoneNumber(Set<? extends PersonPhoneNumber> phoneNumber) {
	...
	public void removeFromBusinessActor(IBusinessActor businessActor) {
		if ( businessActor!=null ) {
			PersonFullfillsActorRole oldLink = getPersonFullfillsActorRole_businessActorFor(businessActor);
			if ( oldLink!=null ) {
				businessActor.z_internalRemoveFromPersonFullfillsActorRole_representedPerson(oldLink);
				oldLink.clear();
				z_internalRemoveFromPersonFullfillsActorRole_businessActor(oldLink);
			}
		}
	}
	...
	public void removeFromBusinessRole(IBusinessRole businessRole) {
		if ( businessRole!=null ) {
			PersonInBusinessRole oldLink = getPersonInBusinessRole_businessRoleFor(businessRole);
			if ( oldLink!=null ) {
				businessRole.z_internalRemoveFromPersonInBusinessRole_representedPerson(oldLink);
				oldLink.clear();
				z_internalRemoveFromPersonInBusinessRole_businessRole(oldLink);
			}
		}
	}
	...
	public void z_internalAddToAuthenticationToken(String authenticationToken) {
		if ( authenticationToken.equals(this.authenticationToken) ) {
			return;
		}
	...
	public void z_internalAddToPhysicalAddress(PhysicalAddress physicalAddress) {
		if ( physicalAddress.equals(this.physicalAddress) ) {
			return;
		}
	...
	public void z_internalAddToPostalAddress(PostalAddress postalAddress) {
		if ( postalAddress.equals(this.postalAddress) ) {
			return;
		}
	...
	public void z_internalAddToTokenExpiryDateTime(Date tokenExpiryDateTime) {
		if ( tokenExpiryDateTime.equals(this.tokenExpiryDateTime) ) {
			return;
		}
