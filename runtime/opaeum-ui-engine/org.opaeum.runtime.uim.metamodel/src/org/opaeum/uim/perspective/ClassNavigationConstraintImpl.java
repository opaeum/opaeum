...
public class ClassNavigationConstraintImpl extends EObjectImpl implements ClassNavigationConstraint {
	private List<BehaviorNavigationConstraint> behaviors = new ArrayList<BehaviorNavigationConstraint>();
	private NavigatorConfiguration explorerConfiguration;
	private boolean hidden;
	private boolean inheritFromParent;
	private Labels labelOverride;
	private String name;
	private Boolean openToPublic;
	private List<OperationNavigationConstraint> operations = new ArrayList<OperationNavigationConstraint>();
	private List<PropertyNavigationConstraint> properties = new ArrayList<PropertyNavigationConstraint>();
	...
	public void buildTreeFromXml(Element xml, Map<String, Object> map) {
		...
		while ( i<propertyNodes.getLength() ) {
			...
			if ( currentPropertyNode instanceof Element && currentPropertyNode.getNodeName().equals("properties") ) {
				String typeString = ((Element)currentPropertyNode).getAttribute("xsi:type");
				PropertyNavigationConstraint curVal;
				if ( typeString==null ||typeString.trim().length()==0 ) {
					typeString="persp:PropertyNavigationConstraint";
				}
				curVal=UimInstantiator.INSTANCE.newInstance(typeString);
			...
			if ( currentPropertyNode instanceof Element && currentPropertyNode.getNodeName().equals("behaviors") ) {
				String typeString = ((Element)currentPropertyNode).getAttribute("xsi:type");
				BehaviorNavigationConstraint curVal;
				if ( typeString==null ||typeString.trim().length()==0 ) {
					typeString="persp:BehaviorNavigationConstraint";
				}
				curVal=UimInstantiator.INSTANCE.newInstance(typeString);
			...
			if ( currentPropertyNode instanceof Element && currentPropertyNode.getNodeName().equals("operations") ) {
				String typeString = ((Element)currentPropertyNode).getAttribute("xsi:type");
				OperationNavigationConstraint curVal;
				if ( typeString==null ||typeString.trim().length()==0 ) {
					typeString="persp:OperationNavigationConstraint";
				}
				curVal=UimInstantiator.INSTANCE.newInstance(typeString);
