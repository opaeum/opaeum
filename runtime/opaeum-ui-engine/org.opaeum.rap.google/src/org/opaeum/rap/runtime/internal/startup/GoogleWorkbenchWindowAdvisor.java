...
import org.opaeum.runtime.jface.ui.IWorkbenchPage;
...
public class GoogleWorkbenchWindowAdvisor extends WorkbenchWindowAdvisor {
	private static final HttpTransport TRANSPORT = new NetHttpTransport();
	private static final JsonFactory JSON_FACTORY = new JacksonFactory();
	private static final Color COLOR_WHITE = Display.getDefault()
			.getSystemColor(SWT.COLOR_WHITE);
	private boolean introActive = true;

	public GoogleWorkbenchWindowAdvisor(final IWorkbenchWindowConfigurer configurer) {
		super(configurer);
	}
	...
	public void preWindowOpen() {
		...
		configurer
				.setTitle(RMSMessages.get().RMSWorkbenchWindowAdvisor_RAPWorkbenchDemo);
		configurer.setShellStyle(SWT.NONE);
		Rectangle bounds = Display.getDefault().getBounds();
		configurer.setInitialSize(new Point(bounds.width, bounds.height));
		String code = RWT.getRequest().getParameter("code");
		IOpaeumApplication application = getApplication();
		String email = application.getEnvironment().getProperty(
				"opaeum.default.user");
		if (email != null) {
			final IPersonNode person = application
					.findOrCreatePersonByEMailAddress(email);
			RWT.getRequest()
					.getSession(true)
					.setAttribute(OpaeumRapSession.class.getName(),
							new OpaeumRapSession(getApplication(), person));
			...
			if (false) {
				...
				GoogleRefreshTokenRequest grant = new GoogleRefreshTokenRequest(
						TRANSPORT, JSON_FACTORY, refreshToken,
						LoginView.CLIENT_ID, OpaeumRapSession.CLIENT_SECRET);
		...
		} else if (RWT.getRequest().getSession(true)
				.getAttribute(OpaeumRapSession.class.getName()) == null
				&& code != null) {
			requestTokens(code);
		}
	}
	...
	private IOpaeumApplication getApplication() {
		return Activator.getDefault().getApplications().values().iterator()
				.next();
	}
	...
	public void postWindowOpen() {
		...
		String code = RWT.getRequest().getParameter("code");
		if (RWT.getRequest().getSession(true).getAttribute("accessToken") == null
				&& code == null
				&& getApplication().getEnvironment().getProperty(
						"opaeum.default.user") == null) {
			// close opened perspective
			IPerspectiveDescriptor pers = PlatformUI.getWorkbench()
					.getActiveWorkbenchWindow().getActivePage()
					.getPerspective();
			PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getActivePage().closePerspective(pers, false, false);
			// open login perspective
			pers = PlatformUI.getWorkbench().getPerspectiveRegistry()
					.findPerspectiveWithId(LoginPerspectiveFactory.ID);
			PlatformUI.getWorkbench().getActiveWorkbenchWindow()
					.getActivePage().setPerspective(pers);
	...
	private void requestTokens(String code) {
		...
		try {
			GoogleAuthorizationCodeTokenRequest authRequest = new GoogleAuthorizationCodeTokenRequest(
					TRANSPORT, JSON_FACTORY, LoginView.CLIENT_ID,
					OpaeumRapSession.CLIENT_SECRET, code,
					LoginView.CALLBACK_URL);
			GoogleTokenResponse authResponse = authRequest.execute();
			GoogleOpaeumRapSession os = new GoogleOpaeumRapSession(getApplication(),
					authResponse.getAccessToken(),
					authResponse.getRefreshToken(),
					authResponse.getExpiresInSeconds());
			os.associatePerson();
			RWT.getRequest().getSession(true)
					.setAttribute(OpaeumRapSession.class.getName(), os);
	...
	private void setSecondaryBannerSize(final Composite secondaryBanner,
		...
		Point location = display.map(clientComposite.getParent(),
				secondaryBanner.getShell(), new Point(1, 1));
		secondaryBanner.setBounds(location.x, location.y, bounds.width,
				bounds.height);
	}
	...
	private void setMainBannerSize(final Composite banner) {
		...
		banner.setBounds(clientArea.x + 3, 0, clientArea.width - 6,
				clientArea.y - 1);
	}
	...
	private void setPageBounds(final IWorkbenchPage page) {
		...
		if (!introActive) {
			...
			clientComposite.getParent().setBounds(bounds.x + 100,
					bounds.y + 100, bounds.width - 100, bounds.height - 100);
		} else {
