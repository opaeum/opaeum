...
import org.topcased.modeler.uml.activitydiagram.policies.ControlFlowEdgeCreationEditPolicy;
import org.topcased.modeler.utils.Utils;

public class OpaeumControlFlowEdgeCreationEditPolicy extends ControlFlowEdgeCreationEditPolicy{
	...
	protected boolean checkSource(GraphElement source){
		if(super.checkSource(source) == false){
			...
			return object instanceof Action || object instanceof ControlNode;
		}
	}
	...
	protected boolean checkTargetForSource(GraphElement source,GraphElement target){
		if(super.checkTargetForSource(source, target) == false){
			...
			if((object instanceof DecisionNode || object instanceof ForkNode) && ((ActivityNode) object).getIncomings().size() > 0){
				return false;
			}
