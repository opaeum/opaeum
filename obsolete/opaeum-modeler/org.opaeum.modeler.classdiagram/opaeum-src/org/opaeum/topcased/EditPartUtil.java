...
public class EditPartUtil{
	public static IAction createDiagramAction(EObject diagramOwner,EObject source,Modeler editor,String id){
		IAction action = null;
		List<Diagram> existingDiagramList = (List<Diagram>) (diagramOwner == null ? Collections.emptyList() : DiagramsUtils
				.findAllExistingDiagram(editor.getDiagrams(), diagramOwner));
	...
	public static IAction createNewDiagram(Modeler editor,EObject modelObject,String id){
		if(TopcasedAdapterFactoryEditingDomain.isEObjectReadOnly(modelObject)){
			MessageDialog.openWarning(ModelerPlugin.getActiveWorkbenchShell(), "Warning",
					"your originalElement is read only you can't create diagrams");
		}else{
