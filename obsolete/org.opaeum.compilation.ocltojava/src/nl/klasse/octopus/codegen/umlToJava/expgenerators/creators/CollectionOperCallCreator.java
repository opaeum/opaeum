...
/**<octel>
 ...
 %inet.sfsf.opaeum.javametamodelametamodel.model.OJBlocknet.sf.sf.opaeum.javametamodell.javametamodel.model.OJIfStanet.sft.sf.opaeum.javametamodelkeduml.javametamodel.model.OJWhnet.sfet.sf.opaeum.javametamodelsf.opaeum.javametamodel.monet.sforg.opaeum.javametamnet.sf org.opaeum.javametamodnet.sfdeorg.opaeum.javametamodenet.sfrt org.opaeum.javametamodel.utilities.JavaPathNames%;
 </java>
 </octel>*/
...
public class CollectionOperCallCreator {
	...
	public String collectionOperCall(OperationCallExp exp, String source,
			List<String> args, IOperation referedOp, boolean isStatic,
			List<OJParameter> params) {
		Check.pre("source is not null", exp.getSource() != null);
		Check.pre("source is collection", exp.getSource().getNodeType()
				.isCollectionKind());
		String result = "";
		//
		if (referedOp != null) {
	...
	private String buildEquals(OperationCallExp exp, String source,
		...
		StdlibCollectionType sourceType = (StdlibCollectionType) exp
	...
	// StdlibCollectionType sourceType = (StdlibCollectionType)
	// exp.getSource().getNodeType();
	// IClassifier elementType = sourceType.getElementType();
	// ClassifierMap elementMap = new ClassifierMap(elementType);
	// String myType;
	...
	private String buildIncluding(OperationCallExp exp, String source,
		...
		if (sourceType.isBag() || sourceType.isSequence()) {
			 ...
			 * <comment> implements %exp.toString()% on
			 * %exp.getSource().toString()% </comment> <body> %returnType%
			 * result = %source%; result.add( %argument% ); return result;
			 * </body> </method> </octel>
			 */
			 ...
			 * <comment> implements %exp.toString()% on
			 * %exp.getSource().toString()% </comment> <body> %returnType%
			 * result = %source%; <if> !result.contains(%argument%)<then>
			 * result.add( %argument% ); </then></if> return result; </body>
			 * </method> </octel>
			 */
	...
	private String buildExcluding(OperationCallExp exp, String source,
		...
		if (sourceType.isBag() || sourceType.isSequence()) {
			 ...
			 * <comment> implements %exp.toString()% on
			 * %exp.getSource().toString()% </comment> <body> %returnType%
			 * result = %source%; <while> result.contains(%argument%) <body>
			 * result.remove( %argument% ); </body></while> return result;
			 * </body> </method> </octel>
			 */
			 ...
			 * <comment> implements %exp.toString()% on
			 * %exp.getSource().toString()% </comment> <body> %returnType%
			 * result = %source%; result.remove( %argument% ); return result;
			 * </body> </method> </octel>
			 */
	...
	private String buildAppend(OperationCallExp exp, String source,
		 ...
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body> %returnType% result = %source%; result.add(
		 * %argStr% ); return result; </body> </method> </octel>
		 */
	...
	private String buildPrepend(OperationCallExp exp, String source,
		 ...
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body> %returnType% result = %source%; result.add( 0,
		 * %argStr% ); return result; </body> </method> </octel>
		 */
	...
	private String buildUnion(OperationCallExp exp, String source,
		 ...
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body octel_condition="%!isUnique%"> %returnType% result =
		 * %source%; result.addAll( %argStr% ); return result; </body> <body
		 * octel_condition="%isUnique%"> %returnType% result = %source%; <for
		 * name="elem" type="%elementTypePath%">
		 * <collection>%argStr%</collection> <body> <if> !result.contains(elem)
		 * <then> result.add(elem); </then></if> </body></for> return result;
		 * </body> </method> </octel>
		 */
	...
	private String buildIntersection(OperationCallExp exp, String source,
		 ...
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body> %returnType% result = %myDefault%; <for name="elem"
		 * type="%myListType%"> <collection>%source%</collection> <body> <if>
		 * %argStr%.contains( elem ) <then> result.add( elem ); </then></if>
		 * </body></for> return result; </body> </method> </octel>
		 */
	...
	private String buildSymmetricDifference(OperationCallExp exp,
		 ...
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body> %returnType% result = %source%;
		 * result.removeAll(%argStr%); <for name="elem" type="%myListType%">
		 * <collection>%argStr%</collection> <body> <if>
		 * !%source%.contains(elem) <then> result.add(elem); </then></if>
		 * </body></for> return result; </body> </method> </octel>
		 */
	...
	private String buildSum(OperationCallExp exp, String source,
		...
		if (!(elementType instanceof StdlibPrimitiveType)) { // the model class
																// defines the
																// '+' operation
		...
		// generate separate operation
		/**
		 * <octel var="myClass"> <method type="%elementMap.javaTypePath()%"
		 * name="%operName%" visibility="%OJVisibilityKind.PRIVATE%"
		 * static="%isStatic%" var="oper"> <paramlist> %params% </paramlist>
		 * <comment> implements %exp.toString()% on %exp.getSource().toString()%
		 * </comment> <body> %myType% result = %myDefault%; <for name="elem"
		 * type="%myListType%"> <collection>%source%</collection> <body> result
		 * = %sumExp%; </body></for> return result; </body> </method> </octel>
		 */
	...
	private String buildExcludes(OperationCallExp exp, String source,
			List<String> args) {
		IClassifier type = ((IOclExpression) exp.getArguments().get(0))
				.getNodeType();
